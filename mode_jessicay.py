...
#print(upd_dir(samples[0][0]))
#print(samples.shape)
#sys.exit()

# Update the address of the images
...
#sys.exit()
print(np.asarray(validation_samples).shape)
    ...
    while 1: # Loop forever so the generator never terminates
        ...
        for offset in range(0, num_samples, batch_size):
            ...
            for batch_sample in batch_samples: 
                # btach_sample represents for each row of the data
                #print(np.asarray(batch_sample).shape)
                #print(batch_sample[0])
                #sys.exit()
                name = batch_sample[0] # Name of the center image
                center_image = mpimg.imread(name)[:,16:304,:]
                print(center_image.shape)  # (160, 320,3)
                #sys.exit()
                label = [float(batch_sample[3]),batch_sample[4],batch_sample[5],batch_sample[6]]
                #print(label)
                #sys.exit()
            ...
            # Should I use shuffle or not?
            return shuffle(X_train, y_train)
           # yield shuffle(X_train, y_train)

# compile and train the model using the generator function
train_set = generator(train_samples, batch_size=32)
validation_set = generator(validation_samples, batch_size=32)
print(train_set[0].shape)
#print(train_generator)
print(train_set[1].shape)
#np.save('train.npy',train_set)
#np.save('test.npy',validation_set)
print(train_set[0].shape)
#sys.exit()
